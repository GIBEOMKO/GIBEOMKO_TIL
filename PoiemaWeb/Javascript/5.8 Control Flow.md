...
<span style='color: red'>`break`</span> 키워드는 코드 블록에서 탈출하는 역할을 수행한다. <span style='color: red'>`break`</span>가 없다면 <span style='color: red'>`case`</span>문의 조건과 일치하지 않더라도 실행 순서는 다음 <span style='color: red'>`case`</span>문으로 이동한다.
